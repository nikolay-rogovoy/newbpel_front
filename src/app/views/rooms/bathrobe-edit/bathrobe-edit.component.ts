...
@Component({
  templateUrl: 'bathrobe-edit.component.html'
})
@ReactiveFormDecorator()
@LoadFormDecorator(EntityName.bathrobe, () => of({ idbathrobe: null, dtcre: new Date(), active: true }))
@SubmitFormDecorator(EntityName.bathrobe)
@DeleteFormDecorator(EntityName.bathrobe)
@ExistsEntityFormDecorator()
@CancelFormDecorator()
@InvalidControlFormDecorator()
export class BathrobeEditComponent extends LifecycleComponent implements IReactiveForm, OnInit {

  formGroup = new FormGroup({
    idbathrobe: new FormControl(''),
